...
describe("validateFlowForActivation — flow-level", () => {
  ...
  it("flags empty node list", () => {
    ...
    expect(
      issues.some((i) => i.message.includes("al menos un nodo")),
    ).toBe(true);
  });
  ...
  it("flags keyword trigger with no keywords", () => {
    ...
    expect(
      issues.some(
        (i) =>
          i.scope === "trigger" &&
          i.message.includes("al menos una palabra clave"),
      ),
    ).toBe(true);
  });
...
describe("validateFlowForActivation — nodes", () => {
    ...
    expect(
      issues.some(
        (i) =>
          i.node_key === "b" &&
          i.field === "buttons" &&
          i.message.includes("al menos un botón"),
      ),
    ).toBe(true);
  });
    ...
    expect(
      issues.some(
        (i) =>
          ...
          i.message.includes("supera los 20"),
      ),
    ).toBe(true);
  });
  ...
  it("flags duplicate button reply_ids", () => {
    ...
    expect(
      issues.some((i) => i.message.includes("reply id de botón duplicado")),
    ).toBe(true);
  });
  ...
  it("flags list row title over 24 chars", () => {
    ...
    expect(
      issues.some((i) => i.message.includes("supera los 24")),
    ).toBe(true);
  });
    ...
    expect(
      issues.some(
        (i) =>
          i.node_key === "orphan" &&
          i.severity === "warning" &&
          i.message.includes("no es alcanzable"),
      ),
    ).toBe(true);
  });
  ...
  it("doesn't crash on unknown node_type — flags it", () => {
    ...
    expect(
      issues.some((i) => i.message.includes("Tipo de nodo desconocido")),
    ).toBe(true);
  });
});
